...
| Var | Default | Notes |
|---|---|---|
| `PARLEY_PORT` | `7539` | Listening port. |
...
You don't have to author this file. `parley-server run` writes a `local` entry on first boot when bound to loopback, and `parley mcp` falls back to `ws://127.0.0.1:7539` even if the file is missing.

## Defaults at a glance

| Concern | Default |
|---|---|
| Listen address | `ws://127.0.0.1:7539` |
