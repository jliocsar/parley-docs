...
Nicknames are unique **per Room**. If the server is auto-allocating a Nickname and hits a collision, it retries with a `-N` suffix; explicit nickname collisions return an error.
...
```jsonc
{
  "joined":    [{ "name": "planning", "nickname": "alice", "membersCount": 3 }],
  "available": [{ "name": "random", "membersCount": 2 }]
}
```
